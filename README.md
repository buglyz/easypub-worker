...
| 项 | 限制 | 说明 |
|---|---|---|
| 上传体积 | 本地转换 **无应用层上限**（浏览器内存为准）；回退云端 `MAX_UPLOAD_BYTES`≈100MB | 前端仅保留 1GB 冒烟护栏防误选；云端 100MB 为 Workers 平台请求体物理上限，由服务器 `parseMultipart` 拦截 |
| 同步阈值 | 8MB（`SYNC_MAX_BYTES`） | 回退云端时超过走异步 job |
| CPU 时间 | 免费档 10ms / 付费档 30s | 仅回退云端时相关；本地转换不受此限（实测 14MB/1593 章同步约 1.9s） |
| 内存 | 本地 = 浏览器内存；云端 ~128MB | 本地为主链路，页面关闭/后台回收会中断本地任务 |
| 产物保留 | 24h | R2 lifecycle 规则，超时自动删除 |

**大小限制仅存在于「回退到 Cloudflare」这一条链**：浏览器不支持 Web Worker、GBK 解码或本地转换异常时，才会把原请求原样交给服务器，此时受 Workers 平台的请求体（~100MB）、内存（~128MB）与 CPU 时间约束。纯本地转换没有应用层限制，真正边界只是浏览器自身内存；前端留的 1GB 检查仅是对多 GB 级误选的保护。
